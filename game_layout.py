...
class GameLayout:
    """
    This class will be used to define the levels grid and check user inputs
    against assigned mines and cell value and return the board to the user.
    """
    ...
    def __init__(self, board_size, no_mines):
        ...
        # use to compare with board_layout on user guess
        self.user_board = self.set_board()
    ...
    def set_board(self):
        ...
        """
        grid = [["_" for col in range(self.board_size)]
    ...
    def reset_gameplay(self, board):
        """
        Reset board function to address validation errors made before any
        guesses or flags placed.
    ...
    def add_mines(self):
        """
        Use import random to select random coordinates in the empty
        board_layout and place mines
    ...
    def add_values(self):
        """
        Loop through grid checking each cells neighboring cells.
        For each neighboring cell with a mine increase mines_detected
        variable by 1.
    ...
    def selected_space(self, x, y, f):
        ...
        Check coordinates against final board
            ...
            - if > 0 return true
            - if 0 use a recursion loop to expand out until all conected 0
            coordinates have been added to guesses list. This will
            never reach a mine as i will always encounter a surounding
            number first.
        Run through the recorded guesses and for each coordinate on the user
        board make it equal to the final board
        ...
        if self.board_layout[x][y] == "*":
            for row in range(self.board_size):
                for col in range(self.board_size):
                    self.user_board[row][col] = str(
                        self.board_layout[row][col])
    ...
    def space_edge_guesses(self, x, y):
        """
        Replace first and last dividers when cells on first and
